...
export const TransactionList: React.FC<TransactionListProps> = ({
    ...
    const tableHeaders = useMemo(() => {
        return ["Date", "Description", "Category", "Amount"].map(
            (header) => (
        ...
        return transactions.map((transaction, index) => (
            <tr key={index} className="border-b border-solid border-b-gray-200 max-sm:min-w-[875px]">
                ...
                <td className="text-center h-[72px]">
                    <div className=" py-0 h-8 text-sm bg-gray-100 rounded-xl text-neutral-900">
    ...
    return (
        <table
            className="rounded-xl border border-solid border-zinc-200 max-sm:overflow-x-auto w-full bg-white border-collapse">
            <thead className="rounded-xl">
            <tr className="bg-white border-b border-solid border-b-gray-200 max-sm:min-w-[875px]">
                { tableHeaders }
            </tr>
            </thead>
            <tbody>
