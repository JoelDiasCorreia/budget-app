...
export const TransactionForm: React.FC<TransactionFormProps> = ({
  ...
  return (
    <div className="flex flex-col items-center px-0 py-5 max-w-[960px] w-[520px] max-md:w-full">
      <h1 className="self-start mb-5 text-3xl font-bold leading-10 text-neutral-900 max-sm:text-2xl">{title}</h1>

      <form onSubmit={handleSubmit} className="flex flex-col px-4 py-3 w-full max-w-[480px] max-sm:px-0 max-sm:py-3">
        <div className="flex flex-col w-full">
          <label className="pb-2 text-base leading-6 text-neutral-900">{amountLabel}</label>
          <input
        ...
        <div className="flex flex-col w-full mt-6">
          <label className="pb-2 text-base leading-6 text-neutral-900">{categoryLabel}</label>
          <select
            disabled={isProcessing}
        ...
        <div className="flex flex-col w-full mt-6">
          <label className="pb-2 text-base leading-6 text-neutral-900">{dateLabel}</label>
          <input
            disabled={isProcessing}
        ...
        <div className="flex flex-col w-full mt-6">
          <label className="pb-2 text-base leading-6 text-neutral-900">{descriptionLabel}</label>
          <textarea
            disabled={isProcessing}
        ...
        <Button disabled={isProcessing} className={`mt-6 h-12 text-base font-bold leading-6 ${buttonColor()}`} type="submit">
          {submitLabel}
        </Button>
      </form>
    </div>
