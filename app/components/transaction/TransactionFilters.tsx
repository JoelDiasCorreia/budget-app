...
export const TransactionFilters: React.FC<FilterProps> = ({ onFilterChange, activeFilter }) => {
      ...
      {filters.map((filter) => (
        <button
          ...
          className={`px-4 py-0 h-8 text-sm rounded-xl cursor-pointer max-sm:shrink-0 ${
            activeFilter === filter ? "bg-blue-600 text-white" : "bg-gray-100 text-neutral-900"
          }`}
        >
          {filter}
        </button>
